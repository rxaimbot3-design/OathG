...
export class AuditLogMonitor {
  ...
  static log(guildId: string, action: string, userId: string, details?: any): void {
    return this.getInstance().log(guildId, action, userId, details);
  }

  static getLogs(guildId?: string, limit = 50): AuditLogEntry[] {
    return this.getInstance().getLogs(guildId, limit);
  }
  ...
  static getLogCount(): number {
    return this.getInstance().getLogCount();
  }
}